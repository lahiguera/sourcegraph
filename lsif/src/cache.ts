...
export class GenericCache<K, V> {
     ...
     * @param now A function used to get the current time in milliseconds. Used only for testing.
     */
    constructor(
        private max: number,
    ...
    private async getEntry(
        ...
        if (node) {
            ...
            if (now - entry.lastUpdateMs > this.validityInterval) {
                ...
                // from concurrent getEntry calls.
...
export class ConnectionCache extends GenericCache<string, Connection> {
    ...
    public withConnection<T>(
        ...
        return this.internalWithValue(
            ...
            async (lastUpdatedMs: number): Promise<boolean> => lastUpdatedMs <= (await fs.stat(database)).mtimeMs,
            callback
        )
    }
